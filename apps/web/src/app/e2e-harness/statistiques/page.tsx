...
export default async function E2EStatistiquesPage() {
  ...
  const recentScans = Array.from({ length: 90 }, (_, i) => ({ scanned_at: iso(i % 30, i % 12), device: devices[i % 5], country: pays[i % 6], page_id: i % 3 ? "demo-page-1" : "demo-page-2", qr_code_id: i % 3 ? "demo-qr-1" : "demo-qr-2" }))
  const recentViews = Array.from({ length: 200 }, (_, i) => ({ viewed_at: iso(i % 30, i % 14), device: devices[i % 5], source: i % 4 ? "qr" : "direct", country: pays[i % 6], page_id: i % 3 ? "demo-page-1" : "demo-page-2" }))
  const blocks = [
  ...
  const clicks = Array.from({ length: 60 }, (_, i) => ({ block_id: ["demo-b1", "demo-b2", "demo-b3"][i % 3], click_target: "https://exemple.fr", clicked_at: iso(i % 30, i % 9), page_id: i % 3 === 2 ? "demo-page-2" : "demo-page-1", block_type: ["cta_button", "menu_section", "social_links"][i % 3] }))
  ...
  const pageEvents = Array.from({ length: 120 }, (_, i) => ({ kind: (["scroll", "impression", "dwell", "tap"] as const)[i % 4], ref: i % 4 === 0 ? String(25 * ((i % 4) + 1)) : "demo-b1", value: i % 4 === 0 ? 25 * ((i >> 2) % 4 + 1) : 3, x: 0.3 + (i % 5) / 10, y: 0.2 + (i % 7) / 10, page_id: "demo-page-1", created_at: iso(i % 30, i % 8) }))
  return (
    <ToastProvider><ConfirmProvider><AnalyticsShell
      profile={{ total_pages: 2, total_scans: 169, plan: "pro", email: "demo@qrowg.fr", full_name: "Studio Horizon (démo)" }}
