...
const NEON = "#39FF8F"
const MUTED = "#8A8478"
const COLORS = [GOLD, NEON, "#7B61FF", "#FF6B6B", "#4ECDC4"]
...
function buildDailyData(scans: Scan[], views: View[]) {
  ...
  scans.forEach(s => { const key = s.scanned_at.slice(0, 10); if (map[key]) map[key].scans++ })
  views.forEach(v => { const key = v.viewed_at.slice(0, 10); if (map[key]) map[key].views++ })
...
function buildSourceData(views: View[]) {
  const counts: Record<string, number> = {}
  views.forEach(v => { const src = v.source || "direct"; counts[src] = (counts[src] || 0) + 1 })
  return Object.entries(counts).map(([name, value]) => ({ name, value }))
}
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 32, flexWrap: "wrap", gap: 16 }}>
          ...
          <select value={selectedPage} onChange={e => setSelectedPage(e.target.value)}
            style={{ background: "#111009", border: "1px solid rgba(201,168,76,0.3)", borderRadius: 8, color: "#F5F0E8", padding: "8px 14px", fontSize: 14 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 16, marginBottom: 32 }}>
          {[
            ...
            { icon: <Eye size={20} />, label: "Vues (30j)", value: filteredViews.length, color: NEON },
            ...
            <div key={i} style={{ background: "#111009", border: "1px solid rgba(201,168,76,0.15)", borderRadius: 12, padding: "20px 24px", display: "flex", alignItems: "center", gap: 16 }}>
              <div style={{ color: kpi.color, background: kpi.color + "18", borderRadius: 8, padding: 10 }}>{kpi.icon}</div>
              <div>
            ...
            <AreaChart data={dailyData}>
              <defs>
                ...
                <linearGradient id="gViews" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="5%" stopColor={NEON} stopOpacity={0.3} /><stop offset="95%" stopColor={NEON} stopOpacity={0} />
                </linearGradient>
              </defs>
        ...
        <div style={{ background: "#111009", border: "1px solid rgba(201,168,76,0.15)", borderRadius: 12, padding: 24 }}>
          ...
          {pages.length === 0
            ? <p style={{ color: MUTED }}>Aucune page creee</p>
                    ...
                    <div key={page.id} style={{ display: "flex", alignItems: "center", gap: 16 }}>
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ height: 6, background: "rgba(255,255,255,0.06)", borderRadius: 3 }}>
                          <div style={{ height: "100%", width: pct + "%", background: "linear-gradient(90deg, " + GOLD + ", " + NEON + ")", borderRadius: 3 }} />
                        </div>
                      </div>
                    </div>
                  )
