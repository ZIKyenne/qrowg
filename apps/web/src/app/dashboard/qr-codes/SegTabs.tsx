      ...
      {items.map((it, i) => {
        ...
        return (
          <button key={it.key} role="tab" aria-selected={on} type="button" onClick={() => onChange(i, it.key)} className="qv-tab"
            style={{ position: "relative", zIndex: 1, background: "transparent", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 6, minHeight: 32, padding: dense ? "6px 10px" : "9px 10px", borderRadius: 9, fontSize, fontWeight: on ? 600 : 500, color: on ? "#e8c877" : "#7d766c", transition: "color .2s ease, font-weight .18s ease", whiteSpace: "nowrap" }}>
