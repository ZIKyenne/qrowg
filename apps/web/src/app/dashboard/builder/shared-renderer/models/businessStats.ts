...
export function businessStatsViewModel(content: Record<string, any> | null | undefined): BusinessStatsViewModel {
  const c = content || {}
  const items = extractIndexed<BusinessStat>(c, plafondDesLignes("business_stats"), (cc, i) => texteUtile(cc[`stat${i}_value`]) ? { icon: cc[`stat${i}_icon`], value: cc[`stat${i}_value`], label: cc[`stat${i}_label`] } : null)
