...
export function servicesListViewModel(content: Record<string, any> | null | undefined): ServicesListViewModel {
  const c = content || {}
  const items = extractIndexed<ServiceListItem>(c, plafondDesLignes("services_list"), (cc, i) => cc[`s${i}_name`] ? { icon: cc[`s${i}_icon`], name: cc[`s${i}_name`], desc: cc[`s${i}_desc`] } : null)
