...
// Lien billetterie optionnel par date (href jugé par destinationUtile). visible = au moins un item (lot v153).
...
export function concertsViewModel(content: Record<string, any> | null | undefined): ConcertsViewModel {
  ...
  const items = extractIndexed<ConcertShow>(c, plafondDesLignes("concerts"), (cc, i) => {
    ...
    return { date: cc[`c${i}_date`], city: cc[`c${i}_city`], venue: cc[`c${i}_venue`], link: { href: destinationUtile(url), external: true, trackTarget: url, visible: !!url } }
  })
