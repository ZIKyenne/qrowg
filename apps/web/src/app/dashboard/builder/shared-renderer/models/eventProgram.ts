// Modèle pur `event_program`. items filtrés sur `title`. visible = au moins un item (lot v153).
...
export function eventProgramViewModel(content: Record<string, any> | null | undefined): EventProgramViewModel {
  const c = content || {}
  const items = extractIndexed<ProgramStep>(c, plafondDesLignes("event_program"), (cc, i) => texteUtile(cc[`s${i}_title`]) ? { time: cc[`s${i}_time`], title: cc[`s${i}_title`], desc: cc[`s${i}_desc`] } : null)
