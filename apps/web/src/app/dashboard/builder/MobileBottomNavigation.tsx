      ...
      {MOBILE_BOTTOM_NAV.map(item => {
        ...
        return (
          <button key={item.id} type="button" role="tab" aria-selected={on} data-nav={item.id}
            ...
            {typeof badge === "number" && badge > 0 && <span style={{ position: "absolute", top: 4, right: "50%", marginRight: -24, background: "var(--accent)", color: "var(--ink-on-accent)", borderRadius: 8, minWidth: 15, height: 15, padding: "0 4px", fontSize: 11.5, fontWeight: 700, display: "inline-flex", alignItems: "center", justifyContent: "center" }}>{badge}</span>}
          </button>
        )
      })}
    </nav>
