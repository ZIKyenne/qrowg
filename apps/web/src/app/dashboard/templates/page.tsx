...
export default function TemplatesPage() {
  ...
  const filtered = TEMPLATES.filter((t: any) => {
    ...
    const matchSearch = !search || t.name.toLowerCase().includes(search.toLowerCase()) || t.tags.some((tag: string) => tag.toLowerCase().includes(search.toLowerCase()))
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#080808", paddingBottom: 120, fontFamily: "DM Sans, sans-serif" }}>
      {/* Header */}
      <div style={{ padding: "40px 24px 0", textAlign: "center", maxWidth: 900, margin: "0 auto" }}>
        <div style={{ display: "inline-flex", alignItems: "center", gap: 7, background: "rgba(201,168,76,0.1)", border: "1px solid rgba(201,168,76,0.2)", borderRadius: 20, padding: "5px 14px", marginBottom: 16 }}>
          ...
          <span style={{ color: G, fontSize: 11, fontWeight: 700, letterSpacing: 1.5, textTransform: "uppercase" }}>{TEMPLATES.length} templates disponibles</span>
        </div>
        ...
        <p style={{ color: MUTED, fontSize: 15, margin: "0 0 28px" }}>Page pre-configuree avec les vrais blocs de ton domaine. Cree en 1 clic, personnalise en 5 minutes.</p>

        {/* Recherche */}
        <div style={{ position: "relative", maxWidth: 380, margin: "0 auto 20px", width: "100%" }}>
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Rechercher un template..."
            style={{ width: "100%", background: "#111009", border: "1px solid rgba(201,168,76,0.15)", borderRadius: 10, padding: "10px 10px 10px 36px", color: "#F5F0E8", fontSize: 13, outline: "none", boxSizing: "border-box" }} />
        </div>

        {/* Filtres Plan */}
        <div style={{ display: "flex", gap: 7, justifyContent: "center", flexWrap: "wrap", marginBottom: 14 }}>
          {[["all", "Tous les plans", "#8A8478"], ["free", "Gratuit ✦", "#8A8478"], ["starter", "Starter ⚡", "#38BDF8"], ["pro", "Pro 🔥 9.99€", "#C9A84C"]].map(([plan, label, color]) => (
            <button key={String(plan)} onClick={() => setActivePlan(String(plan))}
              style={{ background: activePlan === plan ? String(color) + "18" : "transparent", border: "1px solid " + (activePlan === plan ? String(color) + "50" : "rgba(255,255,255,0.07)"), borderRadius: 20, padding: "5px 13px", color: activePlan === plan ? String(color) : MUTED, fontSize: 11, fontWeight: activePlan === plan ? 700 : 400, cursor: "pointer", transition: "all 0.15s" }}>
              {String(label)}
            </button>
          ))}
        </div>
        ...
        <div style={{ display: "flex", gap: 6, justifyContent: "center", flexWrap: "wrap", marginBottom: 36 }}>
          {CATEGORIES.map(cat => (
            <button key={cat} onClick={() => setActiveCategory(cat)}
              style={{ background: activeCategory === cat ? "rgba(201,168,76,0.1)" : "transparent", border: "1px solid " + (activeCategory === cat ? "rgba(201,168,76,0.3)" : "rgba(255,255,255,0.06)"), borderRadius: 16, padding: "4px 11px", color: activeCategory === cat ? G : MUTED, fontSize: 11, cursor: "pointer", transition: "all 0.15s" }}>
              {cat}
            </button>
          ))}
        </div>
      ...
      <div style={{ maxWidth: 1280, margin: "0 auto", padding: "0 20px" }}>
        {filtered.length === 0 ? (
          <div style={{ textAlign: "center", padding: "60px 0", color: MUTED }}>
            ...
            <button onClick={() => { setSearch(""); setActiveCategory("Tous"); setActivePlan("all") }} style={{ marginTop: 12, background: "transparent", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 8, padding: "7px 16px", color: MUTED, fontSize: 12, cursor: "pointer" }}>Réinitialiser les filtres</button>
          </div>
        ) : (
