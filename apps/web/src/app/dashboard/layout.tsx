...
export default async function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", background: "#080808", fontFamily: "DM Sans, sans-serif" }}>
      {/* Sidebar */}
      <aside style={{
        width: 220, background: "#111009", borderRight: "1px solid rgba(201,168,76,0.15)",
        display: "flex", flexDirection: "column", padding: "24px 16px", position: "fixed", top: 0, left: 0, height: "100vh"
      }}>
        {/* Logo */}
        <Link href="/dashboard" style={{ textDecoration: "none", marginBottom: 32 }}>
          <span style={{ fontFamily: "Cormorant Garamond, serif", fontSize: 22, color: "#C9A84C", fontWeight: 700 }}>
            QRfolio
          </span>
        </Link>
        ...
        <nav style={{ flex: 1, display: "flex", flexDirection: "column", gap: 4 }}>
          {navItems.map(({ href, label, icon: Icon }) => (
            <Link key={href} href={href} style={{
              ...
              color: "#8A8478", fontSize: 14, transition: "all 0.15s"
            }}
            onMouseEnter={e => {
        ...
        <div style={{ marginTop: "auto" }}>
          {profile?.plan === "free" && (
            <Link href="/upgrade" style={{
              ...
              color: "#C9A84C", fontSize: 13, fontWeight: 600, marginBottom: 16
            }}>
          ...
          <div style={{ padding: "10px 12px", borderRadius: 8, background: "rgba(255,255,255,0.04)" }}>
            ...
            <p style={{ color: "#8A8478", fontSize: 11, margin: "2px 0 0", textTransform: "capitalize" }}>
